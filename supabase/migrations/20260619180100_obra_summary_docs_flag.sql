-- Adiciona has_protected_docs ao resumo público: true quando existe senha de
-- documentos definida E há documentos no projeto. O front usa para decidir se
-- mostra a seção (borrada) de Notas e comprovantes.
...
begin
  ...
  select jsonb_build_object(
    ...
    'has_protected_docs', (
      exists (select 1 from public.public_shares ps
              where ps.token = p_token and ps.enabled = true and ps.docs_password_hash is not null)
      and exists (select 1 from public.documents d where d.project_id = v_project_id)
    ),
    'categories', (
